...
public class Activator implements BundleActivator {
	...
	public HttpService addingService(ServiceReference<HttpService> ref) {
	    ...
	    try {
		httpService.registerServlet("/ui/*", new ResourceServlet(), null,
			httpService.createDefaultHttpContext());
		httpServices.add(httpService);
	    } catch (Exception e) {
		LOG.error("unable to register /ui context", e);
	    }
	    return httpService;
	}
	...
	private void unregisterContext(final HttpService httpService) {
	    try {
		httpService.unregister("/ui/*");
	    } catch (Exception e) {
		LOG.info("unable to unregister /ui context", e);
	    }
	}
